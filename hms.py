...
from app.models import User, Role, UserRoles, Post, ToDoList, TaskStatusLu, TaskPriorityLu, TaskUrgencyLu, ToDoItem, \
    ToDoItemComments, AddressTypeLu, Address, ContactTypeLu, Contact, UnitOfMeasurementLu, ExpenseCategoryLu,  ExpenseTypeLu, Expenses, ExpenseDetails
...
def populateseeddata():
    ...
    db.session.add_all(
        [role_admin, role_testuser, role_customer, role_super_customer])
    db.session.commit()
    ...
    status_created = TaskStatusLu(
        name='Created', description='A newly created task', style_class='p-3 mb-2 bg-primary text-white')
    status_approved = TaskStatusLu(
        name='Approved', description='An approved task', style_class='p-3 mb-2 bg-secondary text-white')
    status_assigned = TaskStatusLu(
        name='Assigned', description='An assigned task', style_class='p-3 mb-2 bg-warning text-dark')
    status_inprogress = TaskStatusLu(
        name='InProgress', description='A task for which work has started', style_class='p-3 mb-2 bg-info text-white')
    status_done = TaskStatusLu(
        name='Done', description='A task which has been completed', style_class='p-3 mb-2 bg-success text-white')
    status_to_be_refined = TaskStatusLu(
        name='To be refined', description='A task to be refined', style_class='p-3 mb-2 bg-white text-dark')

    db.session.add_all([status_created, status_approved,
                        status_assigned, status_inprogress, status_done])
    db.session.commit()
    ...
    db.session.add_all(
        [admin_todo_list1, admin_todo_list2, testuser_todo_list1])
    db.session.commit()
