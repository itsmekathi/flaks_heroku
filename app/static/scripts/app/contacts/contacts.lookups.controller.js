(function () {
    'use strict';
    angular.module('app')
        .controller('contactsLookupsController', ['$scope', '$log', '$mdDialog', '$http', '$window',
            function ($scope, $log, $mdDialog, $http, $window, toaster) {
                $log.log('Contact Lookups controller initialized');
                $scope.status = '  ';
                $scope.lookupName = '';
                ...
                var deleteItem = function () {
                    $http({
                        ...
                        url: $scope.itemUrl + ($scope.lookupName === 'Contact Types' ? 'types' : 'address_types') + '/' + $scope.currentItemId
